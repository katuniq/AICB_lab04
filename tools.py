...
def search_flights(origin: str, destination: str) -> str:
    ...
    """
    # Tra cuu theo cap (origin, destination)
    flights = FLIGHTS_DB.get((origin, destination))
    # Tra cuu theo cap (destination, origin) neu khong thay
    if not flights:
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    #Loc khach san co gia thap hon max_price_per_night
    ...
    if not filtered_hotels:
        return f"Không tìm thấy khách sạn tại {city} với giá dưới {price_fmt} ₫/đêm. Hãy thử tăng ngân sách."
    result = f"Khách sạn tốt nhất tại {city} (Dưới {price_fmt} ₫):\n"

    for h in filtered_hotels:
        price_fmt = f"{h['price_per_night']:,}".replace(",", ".")
        result += f"- {h['name']} ({h['stars']}⭐): {h['area']} | Giá: {price_fmt} ₫/đêm | Rating: {h['rating']}\n"
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for item in items:
            ...
            expense_details += f"- {name.strip().capitalize()}: {amount_val:,}₫\n".replace(",", ".")
        ...
        result += f"Ngân sách ban đầu: {total_budget:,}₫\n".replace(",", ".")

        if remaining >= 0:
                result += f"Còn lại: {remaining:,}₫\n".replace(",", ".")
        else:
            result += f"Vượt ngân sách {-remaining:,}₫! Cần điều chỉnh lại kế hoạch.".replace(",", ".")
        return result
    except Exception:
        return "Lỗi: Định dạng chuỗi expenses không đúng (Ví dụ đúng: 'vé máy bay: 1000000, phòng: 500000')."
